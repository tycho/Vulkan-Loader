...
// 64-bit XXH3. Mirrored byte-for-byte in Python in scripts/generators/extension_name_hash_generator.py and
// scripts/generators/command_name_hash_generator.py which precompute the hash constants embedded in
// loader/generated/vk_command_name_hashes.h and loader/generated/vk_extension_name_hashes.h
//
// Used as a cheap pre-filter before the strcmp confirmation when resolving entry point names
// (vkGet{Instance,Device}ProcAddr): a hash match still falls through to a switch/case-guarded strcmp before anything is
// returned, so a collision can never produce a wrong lookup - at worst it costs one extra strcmp.
static inline uint64_t loader_hash_string(const char *str) {
    if (!str) return 0;
